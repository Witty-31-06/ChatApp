...
async def write_messages(writer):
    ...
    while usr.status == IN_PVT:
        ...
        if message.strip() == 'QUIT':
            ...
            break
        if message.startswith(r'%file='):
            file_path = message.split('=')[1].strip()
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    file_data = f.read()
                    encoded_file = base64.b64encode(file_data).decode('utf-8')
                    msg_to_send = {'type': FILE_SEND_PVT, 'file': encoded_file, 
        ...
        if message.startswith(r'%file='):
            ...
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    file_data = f.read()
                    encoded_file = base64.b64encode(file_data).decode('utf-8')
                    msg_to_send = {'type': FILE_SEND_GRP, 'file': encoded_file, 
                                   'file_name': os.path.basename(file_path)}
                    msg_string = json.dumps(msg_to_send)
                    msg_string += '\n'
                    msg_bytes = msg_string.encode()
...
async def read_messages(reader, writer):
    while True:
        ...
        elif service_type == FILE_RECV_GRP or service_type == FILE_RECV_PVT:
            file_data = msg['file']
